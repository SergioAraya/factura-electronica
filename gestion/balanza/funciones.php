...
function balanza_add($selecionado = "", $excluir = "") {
    ...
    while ($balanza = mysql_fetch_array($sql)) {
        ...
        echo "<option ";
        if ($selecionado == $balanza[0]) {
            echo " selected ";
        } else {
            echo "";
        }
        if ($excluir == $balanza[0]) {
            echo " disabled ";
        } else {
            echo "";
        }
...
function balanza_registro_pagos_segun_factura($facturas_id) {
    global $conexion;
    $sql = mysql_query(
            "SELECT * FROM balanza WHERE id_factura = '$facturas_id' ", $conexion) or error(__DIR__, __FILE__, __LINE__);
    while ($balanza = mysql_fetch_array($sql)) {

        include "../gestion/balanza/reg/reg.php";
        include "../gestion/balanza/vista/registro_pagos_segun_factura.php";
